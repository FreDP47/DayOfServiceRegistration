...
if (isset($_POST['submitted']) && !empty($_POST['submitted'])) {
    ...
    if (isset($_POST['locDrpDwn']) && !empty($_POST['locDrpDwn'])) {
        $locDrpDwn = $_POST['locDrpDwn'];
    }

    $locDrpDwn = !empty($locDrpDwn) ? "'$locDrpDwn'" : "NULL";
    ...
    if($cityDrpDwn == 12 && ($regDrpDwn == "NULL" || $locDrpDwn == "NULL")){
        ...
        if ($usertype == 's') { //save single user
            ...
            if ($cityDrpDwn == 'other') { //City name entered by user
                $otherCityName = mysqli_real_escape_string($conn, $_POST['otherCityName']);
                $newCityAddSql = "INSERT INTO city
                    (city_name)
                    VALUES
                    (
                        '$otherCityName'
                ...
                if (mysqli_query($conn, $newCityAddSql)) {
                    $newCityId = mysqli_insert_id($conn);
                    $singleUserAddSql = "INSERT INTO users
                        (name,email,vol_bakul,vol_dos,feedback,ideas,location_id,region_id,city_id,phone,
                        institution,age,local_language_comfort,photography,film_making,storytelling,craft,general_queries)
                        VALUES
                        (
                            '$name','$email','$volBakulOptionRadio','$volDosOptionRadio',$feedback,$ideas,
                            $locDrpDwn,$regDrpDwn,'$newCityId','$phone','$institution','$age','$localLangDrpDwn',
                            $singleUserPhotography,$singleUserFilmMaking,$singleUserStorytelling,$singleUserCraft,$gen
                        )";
    
                    if (mysqli_query($conn, $singleUserAddSql)) {
                ...
                $mail->AddAddress($receiverEmailAddress);
                        ...
                        (Joy of Giving Week) on 2nd October. You have chosen to participate in the city of <strong>' . $otherCityName . '</strong>';
    
                        if ($regionName != "") {
                            $emailContent .= ' in <strong>' . substr($regionName, 0, strpos($regionName, '(')) . '(' . $locationName . ')</strong>';
                        }
    
                        $emailContent .= '.<br /><br />Please email us at contactbakul@gmail.com if you would like to deregister 
                        or change your registration details or if you have any queries. We will be getting back to you with more details.<br /><br />
                        ...
                        $smsContent = 'Greetings from Bakul Foundation. Thank you for registering for the Day of Service as part of Daan Utsav (Joy of Giving Week) on 2nd October. You have chosen to participate in the city of ' . $otherCityName;
    
                        if ($regionName != "") {
                            $smsContent .= ' in ' . substr($regionName, 0, strpos($regionName, '(')) . '(' . $locationName . ')';
                        }
    
                        $smsContent .= '. You have been sent a more detailed email on ' . $receiverEmailAddress . '. If any of these details need to be changed, please get in touch with us at contactbakul@gmail.com. We will be getting back to you with more details.';
            ...
            } else { //City name selected from options provided
                $singleUserAddSql = "INSERT INTO users
                    (name,email,vol_bakul,vol_dos,feedback,ideas,location_id,region_id,city_id,phone,
                    institution,age,local_language_comfort,photography,film_making,storytelling,craft,general_queries)
                    VALUES
                    (
                        '$name','$email','$volBakulOptionRadio','$volDosOptionRadio',$feedback,$ideas,
                        $locDrpDwn,$regDrpDwn,'$cityDrpDwn','$phone','$institution','$age','$localLangDrpDwn',
                        $singleUserPhotography,$singleUserFilmMaking,$singleUserStorytelling,$singleUserCraft,$gen
                    )";
                ...
                $mail->AddReplyTo("", "");
                    ...
                    $emailContent = 'Hi,<br /><br />Greetings from Bakul Foundation. 
                    Thank you for registering for the <strong>Day of Service</strong> as part of Daan Utsav 
                    (Joy of Giving Week) on 2nd October. You have chosen to participate in the city of <strong>' . $cityName . '</strong>';
                    ...
                    $smsContent = 'Greetings from Bakul Foundation. Thank you for registering for the Day of Service as part of Daan Utsav (Joy of Giving Week) on 2nd October. You have chosen to participate in the city of ' . $cityName;
            ...
            $groupId = "";
            $isRowThere = false;
            $lastUserIdSql = mysqli_query($conn, "SELECT user_id FROM users ORDER BY user_id DESC LIMIT 1");
            while ($row = $lastUserIdSql->fetch_assoc()) {
                $newId = (int)$row['user_id'] + 1;
                $groupId = "GU" . $newId;
                $isRowThere = true;
            }
            ...
            if ($cityDrpDwn == 'other') { //City name entered by user
                ...
                if (mysqli_query($conn, $newCityAddSql)) {
                    ...
                    $groupUserAddSql = "INSERT INTO users
                        (name,email,vol_bakul,vol_dos,feedback,ideas,location_id,region_id,city_id,phone,
                        ...
                        (
                            '$name','$email','$volBakulOptionRadio','$volDosOptionRadio',$feedback,$ideas,
                            $locDrpDwn,$regDrpDwn,'$newCityId','$phone','$institution','$age','$localLangDrpDwn',
                            '$groupId','1',$groupUserPhotography,$groupUserFilmMaking,$groupUserStorytelling,$groupUserCraft,$gen
                        );";
                    ...
                    for ($index = 0; $index < (int)$groupNo-1; $index++) {
                        ...
                        $groupUserAddSql .= "INSERT INTO users
                            (name,email,location_id,region_id,city_id,phone,
                            institution,age,local_language_comfort,group_id)
                            VALUES
                            (
                                '$groupUserName','$groupUserEmail',$locDrpDwn,$regDrpDwn,
                                '$newCityId','$groupUserPhone','$groupUserInstitutionId',
                                '$groupUserAgeId','$groupUserlocalLangComfortId','$groupId'
                            );";
                    }
                ...
                for ($index = 0; $index < (int)$groupNo-1; $index++) {
                    ...
                    $groupUserAddSql .= "INSERT INTO users
                        ...
                        (
                            '$groupUserName','$groupUserEmail',$locDrpDwn,$regDrpDwn,
                            '$cityDrpDwn','$groupUserPhone','$groupUserInstitutionId',
                            '$groupUserAgeId','$groupUserlocalLangComfortId','$groupId'
                        );";
                }
                ...
                $mail->AddReplyTo("", "");
                        ...
                        $emailContent .= '.<br /><br />Please email us at contactbakul@gmail.com if you would like to deregister 
                        or change your registration details or if you have any queries. We will be getting back to you with more details.<br /><br />
                        You can see pictures of earlier Day of Service celebrations here <a href="https://goo.gl/photos/47eTJdC7AiXBtwQ4A">here</a>. 
                        You can know more about Bakul Foundation by watching a film on Bakul 
                        <a href="https://youtu.be/YiiaarY26zY">here</a> or a <strong>TEDx Talk</strong> 
                        on the idea of Bakul <a href="https://youtu.be/dvdclvxzE6o">here</a>. 
                        You can also check out http://www.bakul.org for more details and 
